  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <div className="stamp-ring-solid flex items-center justify-center w-14 h-14 bg-brand-teal text-white text-lg font-bold mx-auto mb-3">
          BT
        </div>
        <h1 className="font-serif text-2xl font-bold text-brand-ink">Welcome Back</h1>
        <p className="text-sm text-brand-ink/60 mt-1">Sign in to your account</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="text-right mt-1">
            <Link href="/forgot-password" className="text-xs text-brand-teal hover:underline">Forgot password?</Link>
          </div>
        </div>
        {error && <p className="text-sm text-brand-coral">{error}</p>}
        <Button type="submit" loading={loading} className="w-full bg-[#25accd] hover:bg-[#1d8ca8] text-white font-bold text-lg h-12 shadow-lg shadow-[#25accd]/30" size="lg">
          Sign In
        </Button>
      </form>
      ...
      <p className="text-center text-sm text-brand-ink/60">
        ...
        <Link href="/signup" className="text-brand-teal hover:underline font-medium">Sign up</Link>
      </p>
    </div>
  );
}
